...
void write_examen__init(FILE * nasm_file, char * name, int index,int direccion,int load){
	//Cargamos el valor en ebx
	if (load){
		_load_1_operator(nasm_file,direccion);
		fprintf(nasm_file, "mov ebx,eax\n");
	}else{
		fprintf(nasm_file, "mov ebx,0\n");
	}

	// Cargamos la dirección de memoria del elemento del vector.
	fprintf(nasm_file, "mov eax,dword %d\n", index);
	fprintf(nasm_file,"; Cargar en edx la dirección de inicio del vector\n");
	fprintf(nasm_file,"mov dword edx, _%s\n",name);
	fprintf(nasm_file,"; Cargar en eax la dirección del elemento indexado\n");
	fprintf(nasm_file,"lea eax, [edx + eax*4]\n");
	...
	// Pusheamos la dirección de memoria en la que guardarlo y despúés la exp
	fprintf(nasm_file, "push dword eax\n");
	fprintf(nasm_file, "push dword ebx\n");

	write_assign(nasm_file,name,0,1);
}
